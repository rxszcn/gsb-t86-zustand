/**
 * 一次改状态之后：通知进行中新加订阅、监听器里再改状态，裸订阅与带选择器的订阅各被叫几次、拿到的上一次是什么。
 * 跑法：node_modules/.bin/esbuild --bundle --platform=node --format=esm --define:import.meta.env="{}" --log-level=error repro/subscribe-notify.ts --outfile=/tmp/sn.mjs 然后 node /tmp/sn.mjs
 */
import { createStore } from '../src/vanilla.ts'
import { subscribeWithSelector } from '../src/middleware/subscribeWithSelector.ts'
